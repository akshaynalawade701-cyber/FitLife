...
async function analyzeAndRender() {
  try {
    ...
    const sTilt = Number.isFinite(metrics.shoulderTilt) ? metrics.shoulderTilt.toFixed(1)+'° off' : '—';
    const hTilt = Number.isFinite(metrics.hipTilt) ? metrics.hipTilt.toFixed(1)+'° off' : '—';
    const headFwd = Number.isFinite(metrics.forwardHead) ? Math.round(metrics.forwardHead*100)+'%' : '—';
    const torso = (metrics.symmetry && Number.isFinite(metrics.symmetry.torsoDiffPct)) ? Math.round(metrics.symmetry.torsoDiffPct*100)+'% diff' : '—';
    const arms = (limb && Number.isFinite(limb.armDiff)) ? Math.round(limb.armDiff*100)+'% diff' : '—';
    const legs = (limb && Number.isFinite(limb.legDiff)) ? Math.round(limb.legDiff*100)+'% diff' : '—';
    setStatus(`Done · Shoulders ${sTilt} · Hips ${hTilt} · Head forward ${headFwd} · Torso symmetry ${torso} · Arms balance ${arms} · Legs balance ${legs}`);
  } catch (err) {
    console.error(err);
